...
namespace Client
{
    ...
    public static class AutoMapperExtension
    {
        ...
        public static TDestination MapTo<TDestination>(this object source, TDestination destination)
        {
            try
            {
                ...
                lock (Sync)
                {
                    ...
                    if (map != null)
                        return Mapper.Map(source, destination);
                    RegisterMap(sourceType, destinationType);
                }
                return Mapper.Map(source, destination);
            }
            catch (AutoMapperMappingException e)
            {
                ...
                {
                    var exception = innerException as AutoMapperConfigurationException;
                    if (exception != null)
                    {
                        foreach (var error in exception.Errors)
                        {
                            RegisterMap(error.TypeMap.SourceType, error.TypeMap.DestinationType);
                        }
        ...
        /// 初始化映射配置
        /// </summary>
        private static void RegisterMap(Type sourceType, Type destinationType)
        {
            try
            {
